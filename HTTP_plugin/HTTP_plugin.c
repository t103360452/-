...
protocol_init(int accept_port, int server_port ATS_UNUSED)
{
  ...
  ret_val = TSTextLogObjectCreate("HTTP_plugin", TS_LOG_MODE_ADD_TIMESTAMP, &protocol_plugin_log);
...
TSPluginInit(int argc, const char *argv[])
{
  ...
  if (argc < 3) {
    TSDebug("HTTP_plugin", "Usage: protocol.so accept_port server_port");
    ...
    if (*end == '\0') {
      accept_port = tmp;
      TSDebug("HTTP_plugin", "using accept_port %d", accept_port);
    ...
    if (*end == '\0') {
      server_port = tmp;
      TSDebug("HTTP_plugin", "using server_port %d", server_port);
